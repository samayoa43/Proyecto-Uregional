...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $sql_docentes = "INSERT INTO docentes (nombres, apellidos, correo, contraseña, id_usuario) VALUES (?, ?, ?, ?, ?)";
        $stmt_docentes = $conexion->prepare($sql_docentes);
        $stmt_docentes->execute([$nombres, $apellidos, $correo, $contraseña, $id_usuario_nuevo]);

        $conexion->commit();

        echo "<div style='font-family: Arial; margin: 20px;'>";
        echo "<h3 style='color: green;'>¡El docente ha sido registrado y enlazado exitosamente!</h3>";
        echo "<a href='formulario_registro_d.php' style='text-decoration: none; background: #0056b3; color: white; padding: 10px; border-radius: 5px;'>Volver al panel</a>";
        echo "</div>";
        ...
        echo "<div style='font-family: Arial; margin: 20px;'>";
        echo "<h3 style='color: red;'>Error al intentar guardar. No se registró al docente.</h3>";
        echo "<p>Detalle técnico: " . htmlspecialchars($e->getMessage()) . "</p>";
        echo "<a href='formulario_registro_d.php'>Volver a intentar</a>";
        echo "</div>";
    }
